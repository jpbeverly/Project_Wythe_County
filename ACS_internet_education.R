...
.key <- "9fb41fe9979018809a049069659efa1df8810000"
...
acs_VA_county_wide<-acs_VA_county_wide%>%
  mutate(PerBroadband=S2801_C02_014)%>%
  mutate(PerHasComputer=round((((S2801_C01_002)/S2801_C01_001)*100),3))%>%
  mutate(PerLess9=DP02_0059P)%>%
  mutate(PerSomeHs=DP02_0060P)%>%
  mutate(PerHsGrad=DP02_0061P)%>%
  mutate(PerSomeColl=DP02_0062P)%>%
  mutate(PerAssoc=DP02_0063P)%>%
  mutate(PerBach=DP02_0064P)%>%
  mutate(PerGradProf=DP02_0065P)%>%
  rename("Code"="GEOID")
...
myvars <- c("Code", "County", "State", "PerBroadband", "PerHasComputer", "PerLess9", "PerSomeHs", "PerHsGrad","PerSomeColl", "PerAssoc", "PerBach","PerGradProf")
newdata <- acs_VA_county_wide[myvars]
wantedCounties <- newdata[c(11,18,38,69,74,83,91,31,68,33,60,44,117,103,109,124,94),] 

write.csv(wantedCounties, "Internet_Education.csv", row.names = FALSE)
